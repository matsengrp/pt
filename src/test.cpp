...
TEST_CASE("Partition", "[partition]") {
  ...
  p_newton->MakeTables(1.5, logl, p_newton->tree_, good_, bad_, vec_thread_);

  for (int i = 0; i < vec_thread_.size(); i++) {
    if (vec_thread_.at(i).joinable())
      vec_thread_.at(i).join();
  }
...
TEST_CASE("NNI_Recursion", "[nnirecursion]") {
  ...
  p_five->MakeTables(1.022081783, logl, p_five->tree_, good_, bad_,
                     vec_thread_);
